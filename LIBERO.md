...
#默认会运行500次测试 task*episode
#setting --center_crop True很重要，因为训练的时候使用了随机裁剪（原图->随机取90%面积->resize回原尺寸->送入模型）来增加鲁棒性。但是测试阶段不能随机裁剪，会导致结果不稳定，所以固定中心90%
...
#no noop是几乎什么都没做的动作，训练价值不大，作者直接过滤掉了。因为机器人数据有大量等待状态，如果全部保留，模型很容易学会大部分输出接近0，从而导致动作塌缩

#记得调整GPU的数量 --nproc-per-node X
#每张GPU batch_size=8，1 需要≈62GB，25GB 显存  
...
#用 torchrun 启动多卡 LoRA 微调.
#注意此处指定了数据集.
#--batch_size 8 指每张 GPU 的 batch size 是 8，不是总 batch size.
#LoRA 的秩为 32。也就是不全量更新 7B 模型，而是只训练低秩适配器.
#训练的时候记得改run_id_note这个名字
#要求训练和测试用同一种 GPU
#LoRA merge：如果在 H100 上训练，但要在 A100 上测试，作者建议：在测试设备上把 LoRA 权重合并进 base model。脚本是vla-scripts/merge_lora_weights_and_save.py。LoRA 训练时模型结构是：base model + LoRA adapter。merge 后变成：新的 base model。好处是推理更稳定，也可能减少跨设备差异。
