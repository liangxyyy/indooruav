...
# 如果某个函数返回tuple，就只返回tuple的第一个元素，否则返回原结果
...
# 提取图像特征的视觉骨干网络，支持单一骨干网络（如 SigLIP）和融合骨干网络（如 SigLIP + DINOv2）配置
# 输入 RGB (B, C, H, W) 图像，输出 (B, num_patches, embed_dim) 特征。例如(B,3,224,224) =>> (B,256,2048)。
# 如果使用两个骨干网络，则输出 (B, num_patches, embed_dim1 + embed_dim2) 特征。
class PrismaticVisionBackbone(nn.Module):
    ...
    def _create_featurizer(self, model_id: str, img_size: int, act_layer: Optional[str]) -> nn.Module:
        ...
        #False代表不直接从Timm下载预训练权重，权重会从OpenVLA的checkpoint中加载。=0代表不要分类头，只要特征。

        # Monkey-patch the forward function to extract the second-to-last layer features
        # 只取倒数第二层特征。因为通常最后一层更偏向预训练任务目标，倒数第二层保留更多通用视觉信息。VLA里面更需要可迁移的空间/语义特征，而不是分类输出
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        """
        # 1.单图+单backbone:(B,3,H,W) → (B,N,D)
        if self.num_images_in_input == 1:
            if not self.use_fused_vision_backbone:
                return self.featurizer(pixel_values)

            # Split `pixel_values :: [bsz, 2 * 3, resolution, resolution]` =>> featurize =>> channel stack
            # 2.单图 + fused backbone:(B,3,H,W)+(B,3,H,W) → (B,6,H,W) → (B,N,D1+D2)
            img, img_fused = torch.split(pixel_values, [3, 3], dim=1)
            patches, patches_fused = self.featurizer(img), self.fused_featurizer(img_fused)

            return torch.cat([patches, patches_fused], dim=2)

        #3.多图 + fused backbone: (B,6*num_images,H,W) → (B,N*num_images,D1+D2)
        else:
...
# 把视觉特征投影到语言模型的嵌入空间中，支持单一骨干网络和融合骨干网络配置
# 对齐维度 fc1->GELU->fc2
class PrismaticProjector(nn.Module):
    def __init__(self, use_fused_vision_backbone: bool, vision_dim: int, llm_dim: int) -> None:
        ...
        # linear layer1: vision_dim -> llm_dim     GELU      linear layer2: llm_dim -> llm_dim 
        if not self.use_fused_vision_backbone:
            self.fc1 = nn.Linear(self.vision_dim, self.llm_dim, bias=True)
            self.fc2 = nn.Linear(self.llm_dim, self.llm_dim, bias=True)
            self.act_fn1 = nn.GELU()
        else:
            #先升维度，因为融合了两个backbone的特征，维度更大，直接映射到llm_dim可能损失信息，所以先升维再降维
...
class PrismaticCausalLMOutputWithPast(ModelOutput):
    ...
    # 每一层Transformer的隐藏状态，包含了每一层的输出特征
    hidden_states: Optional[Tuple[torch.FloatTensor, ...]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None

    # Additions for VLMs
    # 视觉patch 映射到LLM嵌入空间的特征，形状为 (B, num_patches, llm_dim)
...
# 这是整个VLA。它等于Vison + LLM + Projector。它是一个完整的VLM模型，支持条件生成任务。
# 产生高质量的多模态隐藏状态
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    # 把序列中 action token 位置的 embedding 替换成 noisy action embedding。
    ...
    def _build_multimodal_attention(self, input_embeddings, projected_patch_embeddings, attention_mask):
        ...
        #视觉token的mask是1，表示它们参与attention，文本token的mask是原来的mask，拼接起来就是多模态的mask

        return multimodal_embeddings, multimodal_attention_mask

    # 这个函数是把视觉 token 的 label 设置为 IGNORE_INDEX，表示它们不参与 loss 计算。文本也是不参与loss计算的，只有动作 token 参与 loss 计算。
...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    # 这个函数是推理时构造动作槽位。这些 placeholder 本身不是动作，它们只是给 Transformer 留出 56 个位置，用来产生动作 hidden states。
    ...
    # 推理时其实没有真实 label，但代码需要通过 label 找 action mask。所以它构造 fake labels：
