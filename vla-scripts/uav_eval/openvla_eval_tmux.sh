...
tmux set-environment -t "${SESSION_NAME}" NUM_ACTION_BRANCHES "${NUM_ACTION_BRANCHES}"
tmux set-environment -t "${SESSION_NAME}" ACTION_BRANCH_INDEX "${ACTION_BRANCH_INDEX}"
...
tmux send-keys -t "${SESSION_NAME}:model" \
  "cd ${ROOT_DIR} && CHECKPOINT='${CHECKPOINT}' CONDITION_THRESHOLD='${CONDITION_THRESHOLD}' USE_CONDITION_PLAN='${USE_CONDITION_PLAN}' USE_COND_ACTION_TOKENS='${USE_COND_ACTION_TOKENS}' NUM_ACTION_BRANCHES='${NUM_ACTION_BRANCHES}' ACTION_BRANCH_INDEX='${ACTION_BRANCH_INDEX}' bash online_eval/vla_eval/run_openvla_model_runner.sh 2>&1 | tee ${MODEL_LOG}" \
  C-m
