...
ALPHA = 1  # NDTW参数
DEFAULT_INDOOR_UAV_BASE = "/VLM/datasets/Indoor_UAV"
...
def process_episode(trajectory_file, indoor_uav_base):
    """处理单个episode文件"""
    try:
        # 加载轨迹文件
        ...
        vla_ins_path = os.path.join(
            indoor_uav_base, "vla_ins", scene_name, env_name, traj_folder, vla_file
        )
        ...
        posture_path = os.path.join(
            indoor_uav_base, "without_screenshot", scene_name, env_name, traj_folder, "posture.json"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate IndoorUAV-VLA trajectories with paper metrics.")
    ...
    total_count = 0
    total_nDTW = 0.0
    total_position_nDTW = 0.0
    total_yaw_nDTW = 0.0

    # 遍历所有轨迹文件
    for filename in sorted(os.listdir(trajectories_dir)):
        if filename.endswith(".json") and filename != "final_results.json":
            ...
            if result:
                ...
                total_position_nDTW += result["nDTW_position"]
                total_yaw_nDTW += result["nDTW_yaw"]
    ...
    with open(output_file, 'w') as f:
        json.dump({
            ...
            "overall_metrics": {
                ...
                "average_position_nDTW": avg_position_nDTW,
                "average_yaw_nDTW": avg_yaw_nDTW,
    ...
    print(f"Average position nDTW: {avg_position_nDTW:.4f}")
    print(f"Average yaw nDTW: {avg_yaw_nDTW:.4f}")
